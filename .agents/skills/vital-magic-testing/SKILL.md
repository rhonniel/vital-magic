...
description: Crear, corregir y mantener tests Java de vital-magic con JUnit 5, Mockito, WebMvcTest, DataJpaTest y E2E con SpringBootTest + HTTP real + MySQL/Testcontainers, siguiendo sus capas y convenciones. Usar al añadir cobertura o diagnosticar fallos de tests de este repositorio.
---
...
Lee el `AGENTS.md` aplicable, el código de producción afectado y el test vecino de la misma capa. Confirma `pom.xml` y, para persistencia, `src/test/java/com/lps/vitalMagic/config/MySqlIntegrationTest.java`. El checkout actual prevalece sobre esta guía: no des por aplicado un cambio mencionado en una conversación.
...
| Consultas, filtros, proyecciones y SQL | DataJpaTest + MySqlIntegrationTest | [JPA/MySQL](references/jpa-tests.md) |
...
| Flujo crítico completo desde HTTP hasta persistencia y efectos de negocio | SpringBootTest con RANDOM_PORT + MySQL/Testcontainers | [E2E](references/e2e-tests.md) |

Carga solo las referencias correspondientes. Un cambio puede necesitar dos capas si prueba contratos diferentes. Prueba cada comportamiento en la capa más pequeña adecuada. `@SpringBootTest` NO es la plantilla general: resérvalo para pocos flujos completos de alto valor. Unit tests, MVC y JPA siguen siendo las herramientas principales para cobertura detallada. No mezcles slices MVC/JPA en una clase.
...
Pocos escenarios y happy paths críticos; uno o dos fallos adicionales solo si protegen propiedades transversales importantes. Sin mocks internos ni `@Transactional` para rollback automático; limpiar explícitamente los datos committed. Verificar persistencia con repositories o `JdbcTemplate`. No modificar producción para assertions ni cambiar relaciones `LAZY` a `EAGER`. Los detalles operativos están en [E2E](references/e2e-tests.md).
...
4. Ejecuta el conjunto mínimo indicado en [ejecución](references/execution.md). Ejecuta cada E2E nuevo individualmente y después con el conjunto E2E relevante para detectar contaminación de datos o problemas al compartir el contenedor. Si cambias la base MySQL, comprueba además varias clases JPA juntas.
